...
#df <- as_tibble(read.csv('Final_Traitofinterest_ph.csv'))
...
ci_extra_params4 <- Boot(fit_nlsLM4, f = function(x){unlist(calc_params(x))}, 
                    labels = names(calc_params(fit_nlsLM4)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
cl.alT <- rbind(topt7,topt8)

cl.alT$param <- as.character(c("1/development time","juvenile mortality rate"))

cl.alT$euler_param <- as.character(c("1/alpha","zj"))

cl.alT$stage <- as.factor(c("juvenile","juvenile"))

cl.alT$stage <- relevel(cl.alT$stage, "juvenile")

moz1 <- rbind(aedes,cl.alT)

ggplot(moz1, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz2 <- rbind(moz1,an.gT)

ggplot(moz2, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz3 <- rbind(moz2,ap.gT)

ggplot(moz3, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz4 <- rbind(moz3,te.iT)

ggplot(moz4, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
ggplot(ppp_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, ag.zj)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  labs(x = 'Temperature (?C)',
...
moz5 <- rbind(moz4,ag.iT)

ggplot(moz5, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz6 <- rbind(moz5,acc.accT)


ggplot(moz6, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz7 <- rbind(moz6,acp.acpT)

ggplot(moz7, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
ha.dv <- ha.i %>% filter(originaltraitname == "Juvenile Development Rate")

ha.dv <- ha.dv %>% select(originaltraitvalue,temp)
ha.dv <- ha.dv %>% rename(rate = originaltraitvalue)
...
fit_nlsLM24x <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                 data = ha.bpk,
                                 start = coef(haf_fits$pawar[[1]]),
                                 weights = rep(1, times = nrow(ha.bpk)))

# bootstrap using case resampling
boot24x <- Boot(fit_nlsLM24x, method = 'residual')

boot24x_preds <- boot24x$t %>%
  as.data.frame() %>%
  drop_na() %>%
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
...
# calculate bootstrapped confidence intervals
boot24x_conf_preds <- group_by(boot24x_preds, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  ...
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot24x_conf_preds, fill = 'blue', alpha = 0.3) +
...
extra_params24x <- calc_params(fit_nlsLM24x) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params24x <- Boot(fit_nlsLM24x, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLM24x)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_params24x <- left_join(ci_extra_params24x, extra_params24x)

#- Topt 

topt24x <- as_tibble(ci_extra_params24x[2,])
topt24x$species <- as.character("Helicoverpa armigera")

ggplot(topt24x, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
ha.accT <- rbind(topt23,topt24,topt24x)
...
moz8 <- rbind(moz7,ha.accT)

ggplot(moz8, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz9 <- rbind(moz8,jc.accT)

ggplot(moz9, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
ggplot(mi.accT, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
moz10 <- rbind(moz9,mi.accT,mi2.iT)

ggplot(moz10, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
ci_extra_params28 <- Boot(fit_nlsLM28, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLM28)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
moz11 <- rbind(moz10,pc.accT)

ggplot(moz11, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
s.p <- df %>% filter(species == "Stethorus punctillum", standardisedtraitname != "NA")

#- Development Rate

sp.dv <- s.p %>% filter(standardisedtraitname == "Juvenile Development Rate")

sp.dv <- sp.dv %>% select(standardisedtraitvalue,temp)
sp.dv <- sp.dv %>% rename(rate = standardisedtraitvalue)

sp.dv$rate <- sp.dv$rate + 10^-6


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(sp.dv$temp, sp.dv$rate, model_name = 'pawar_2018')

ax_fits <- nest(sp.dv, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
ax_preds <- mutate(ax_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(ax_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, sp.dv)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMa <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = sp.dv,
                                start = coef(ax_fits$pawar[[1]]),
                                lower = get_lower_lims(sp.dv$temp, sp.dv$rate, model_name = 'pawar_2018'),
                                upper = get_upper_lims(sp.dv$temp, sp.dv$rate, model_name = 'pawar_2018'),
                                weights = rep(1, times = nrow(sp.dv)))

# bootstrap using case resampling
boota <- Boot(fit_nlsLMa, method = 'residual')
...
boota_preds <- boota$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(sp.dv$temp), max(sp.dv$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
boota_conf_preds <- group_by(boota_preds, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), ax_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boota_conf_preds, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), sp.dv, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
       y = 'dev rate')
...
extra_paramsa <- calc_params(fit_nlsLMa) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsa <- Boot(fit_nlsLMa, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMa)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsa <- left_join(ci_extra_paramsa, extra_paramsa)

#- Topt 

topta <- as_tibble(ci_extra_paramsa[2,])
topta$species <- as.character("Stethorus punctillum")

ggplot(topta, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
sp.zj <- s.p %>% filter(standardisedtraitname == "Juvenile Mortality Rate")

sp.zj <- sp.zj %>% select(standardisedtraitvalue,temp)
sp.zj <- sp.zj %>% rename(rate = standardisedtraitvalue)

sp.zj$rate <- 1/sp.zj$rate


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(sp.zj$temp, sp.zj$rate, model_name = 'pawar_2018')

ay_fits <- nest(sp.zj, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
ay_preds <- mutate(ay_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(ay_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, sp.zj)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMb <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = sp.zj,
                                start = coef(ay_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(sp.zj)))

# bootstrap using case resampling
bootb <- Boot(fit_nlsLMb, method = 'residual')
...
bootb_preds <- bootb$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(sp.zj$temp), max(sp.zj$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
bootb_conf_preds <- group_by(bootb_preds, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), ay_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), bootb_conf_preds, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), sp.zj, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
       y = '1/zj')
...
extra_paramsb <- calc_params(fit_nlsLMb) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsb <- Boot(fit_nlsLMb, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMb)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsb <- left_join(ci_extra_paramsb, extra_paramsb)

#- Topt 

toptb <- as_tibble(ci_extra_paramsb[2,])
toptb$species <- as.character("Stethorus punctillum")

ggplot(toptb, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
t.m <- df %>% filter(species == "Tetranychus mcdanieli", standardisedtraitname != "NA")

#- Development Rate

tm.dv <- t.m %>% filter(standardisedtraitname == "Juvenile Development Rate")

tm.dv <- tm.dv %>% select(standardisedtraitvalue,temp)
tm.dv <- tm.dv %>% rename(rate = standardisedtraitvalue)

tm.dv$rate <- tm.dv$rate + 10^-6


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tm.dv$temp, tm.dv$rate, model_name = 'pawar_2018')

az_fits <- nest(tm.dv, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
az_preds <- mutate(az_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(az_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tm.dv)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMc <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tm.dv,
                                start = coef(az_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tm.dv)))

# bootstrap using case resampling
bootc <- Boot(fit_nlsLMc, method = 'residual')
...
bootc_preds <- bootc$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tm.dv$temp), max(tm.dv$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
bootc_conf_preds <- group_by(bootc_preds, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), az_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), bootc_conf_preds, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tm.dv, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
       y = '1/zj')
...
extra_paramsc <- calc_params(fit_nlsLMc) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsc <- Boot(fit_nlsLMc, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMc)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsc <- left_join(ci_extra_paramsc, extra_paramsc)

#- Topt 

toptc <- as_tibble(ci_extra_paramsc[2,])
toptc$species <- as.character("Tetranychus mcdanieli")

ggplot(toptc, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
tm.zj <- t.m %>% filter(standardisedtraitname == "Juvenile Mortality Rate")

tm.zj <- tm.zj %>% select(standardisedtraitvalue,temp)
tm.zj <- tm.zj %>% rename(rate = standardisedtraitvalue)

tm.zj$rate <- 1/tm.zj$rate


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tm.zj$temp, tm.zj$rate, model_name = 'pawar_2018')

bx_fits <- nest(tm.zj, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
bx_preds <- mutate(bx_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(bx_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tm.zj)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLMd <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tm.zj,
                                start = coef(bx_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tm.zj)))

# bootstrap using case resampling
bootd <- Boot(fit_nlsLMd, method = 'residual')
...
bootd_preds <- bootd$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tm.zj$temp), max(tm.zj$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
bootd_conf_preds <- group_by(bootd_preds, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), bx_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), bootd_conf_preds, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tm.zj, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
       y = '1/zj')
...
extra_paramsd <- calc_params(fit_nlsLMd) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsd <- Boot(fit_nlsLMd, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMd)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsd <- left_join(ci_extra_paramsd, extra_paramsd)

#- Topt 

toptd <- as_tibble(ci_extra_paramsd[2,])
toptd$species <- as.character("Tetranychus mcdanieli")

ggplot(toptd, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
T.Tb <- rbind(topta,toptb,toptc,toptd)

T.Tb$param <- as.character(c("1/development time","juvenile mortality rate","1/development time","juvenile mortality rate"))

T.Tb$euler_param <- as.character(c("1/alpha","zj","1/alpha","zj"))

T.Tb$stage <- as.factor(c("juvenile","juvenile","juvenile","juvenile"))

ggplot(T.Tb, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
t.b <- df %>% filter(species == "Trichogramma bruni")

#- Development Rate

tb.dv <- t.b %>% filter(standardisedtraitname == "Juvenile Development Rate")

tb.dv <- tb.dv %>% select(standardisedtraitvalue,temp)
tb.dv <- tb.dv %>% rename(rate = standardisedtraitvalue)

tb.dv$rate <- tb.dv$rate + 10^-6


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tb.dv$temp, tb.dv$rate, model_name = 'pawar_2018')

az_fits <- nest(tb.dv, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
ggplot(az_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tb.dv)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMe <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tb.dv,
                                start = coef(az_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tb.dv)))

# bootstrap using case resampling
boote <- Boot(fit_nlsLMe, method = 'residual')
...
boot_predse <- boote$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tb.dv$temp), max(tb.dv$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
boot_conf_predse <- group_by(boot_predse, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), az_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot_conf_predse, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tb.dv, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (C)',
       y = '1/zj')
...
extra_paramse <- calc_params(fit_nlsLMe) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramse <- Boot(fit_nlsLMe, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMe)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramse <- left_join(ci_extra_paramse, extra_paramse)

#- Topt 

topte <- as_tibble(ci_extra_paramse[2,])
topte$species <- as.character("Trichogramma bruni")

ggplot(topte, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
t.l <- df %>% filter(species == "Trichogramma sp. nr. Lutea")

#- Development Rate

tl.dv <- t.l %>% filter(standardisedtraitname == "Juvenile Development Rate")

tl.dv <- tl.dv %>% select(standardisedtraitvalue,temp)
tl.dv <- tl.dv %>% rename(rate = standardisedtraitvalue)

tl.dv$rate <- tl.dv$rate + 10^-6


# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tl.dv$temp, tl.dv$rate, model_name = 'pawar_2018')

start_vals[3] <- 17.01

cz_fits <- nest(tl.dv, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
cz_preds <- mutate(cz_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(cz_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tl.dv)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMf <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tl.dv,
                                start = coef(cz_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tl.dv)))

# bootstrap using case resampling
bootf <- Boot(fit_nlsLMf, method = 'residual')
...
boot_predsf <- bootf$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tl.dv$temp), max(tl.dv$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
boot_conf_predsf <- group_by(boot_predsf, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), cz_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot_conf_predsf, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tl.dv, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (C)',
       y = '1/zj')
...
extra_paramsf <- calc_params(fit_nlsLMf) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsf <- Boot(fit_nlsLMf, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMf)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsf <- left_join(ci_extra_paramsf, extra_paramsf)

#- Topt 

toptf <- as_tibble(ci_extra_paramsf[2,])
toptf$species <- as.character("Trichogramma sp. nr. lutea")

ggplot(toptf, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
T.Trb <- rbind(topte,toptf)

T.Trb$param <- as.character(c("1/development time","1/development time"))

T.Trb$euler_param <- as.character(c("1/alpha"))

T.Trb$stage <- as.factor(c("juvenile","juvenile"))

jq <- rbind(T.Tb,T.Trb)

ggplot(jq, aes(estimate, species, shape = euler_param,colour=stage)) +
  geom_point(size = 2.5) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
t.m <- df %>% filter(species == "Tetraneura nigri abdominalis")
...
cx_fits <- nest(tm.dv, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
cx_preds <- mutate(cx_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
fit_nlsLMg <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tm.dv,
                                start = coef(cx_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tm.dv)))

# bootstrap using case resampling
bootg <- Boot(fit_nlsLMg, method = 'residual')
...
# calculate bootstrapped confidence intervals
boot_conf_predsg <- group_by(boot_predsg, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), cx_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot_conf_predsg, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tm.dv, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (C)')
...
extra_paramsg <- calc_params(fit_nlsLMg) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsg <- Boot(fit_nlsLMg, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMg)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsg <- left_join(ci_extra_paramsg, extra_paramsg)

#- Topt 

toptg <- as_tibble(ci_extra_paramsg[2,])
toptg$species <- as.character("Tetraneura nigri abdominalis")

ggplot(toptg, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
tm.z <- t.m %>% filter(standardisedtraitname == "Female Mortality Rate")

tm.z <- tm.z %>% select(standardisedtraitvalue,temp)
tm.z <- tm.z %>% rename(rate = standardisedtraitvalue)

tm.z$rate <- 1/tm.z$rate

# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tm.z$temp, tm.z$rate, model_name = 'pawar_2018')

cy_fits <- nest(tm.z, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
cy_preds <- mutate(cy_fits, new_data = map(data, ~tibble(temp = seq(min(.x$temp), max(.x$temp), length.out = 100))))%>% 
  # get rid of original data column
  select(., -data) %>%
  # stack models into a single column, with an id column for model_name
  pivot_longer(., names_to = 'model_name', values_to = 'fit', c(pawar)) %>%
...
ggplot(cy_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tm.z)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMh <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tm.z,
                                start = coef(cy_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tm.z)))

# bootstrap using case resampling
booth <- Boot(fit_nlsLMh, method = 'residual')
...
boot_predsh <- booth$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tm.z$temp), max(tm.z$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
boot_conf_predsh <- group_by(boot_predsh, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), cy_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot_conf_predsh, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tm.z, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (C)')

# calculate params with CIs

extra_paramsh <- calc_params(fit_nlsLMh) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsh <- Boot(fit_nlsLMh, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMh)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsh <- left_join(ci_extra_paramsh, extra_paramsh)

#- Topt 

topth <- as_tibble(ci_extra_paramsh[2,])
topth$species <- as.character("Tetraneura nigri abdominalis")

ggplot(topth, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
tm.f <- t.m %>% filter(originaltraitname == "Fecundity Rate")

tm.f <- tm.f %>% select(originaltraitvalue,temp)
tm.f <- tm.f %>% rename(rate = originaltraitvalue)

tm.f$rate <- tm.f$rate + 10^-6

# fit chosen model formulation in rTPC

start_vals <- get_start_vals(tm.f$temp, tm.f$rate, model_name = 'pawar_2018')

cz_fits <- nest(tm.f, data = c(temp, rate)) %>%
  mutate(pawar = map(data, ~nls_multstart(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                          data = .x,
                                          iter = c(3,3,3,3),
                                          start_lower = start_vals - 10,
...
ggplot(cz_preds) +
  geom_line(aes(temp, .fitted, col = model_name)) +
  geom_point(aes(temp, rate),size=0.2,alpha=0.5, tm.f)+
  theme_bw() +
  theme(legend.position = 'none') +
  scale_color_brewer(type = 'qual', palette = 2) +
  theme_bw(base_size = 10)+
...
fit_nlsLMi <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                                data = tm.f,
                                start = coef(cz_fits$pawar[[1]]),
                                weights = rep(1, times = nrow(tm.f)))

# bootstrap using case resampling
booti <- Boot(fit_nlsLMi, method = 'residual')
...
boot_predsi <- booti$t %>%
  ...
  mutate(iter = 1:n()) %>%
  group_by_all() %>%
  do(data.frame(temp = seq(min(tm.f$temp), max(tm.f$temp), length.out = 100))) %>%
  ungroup() %>%
  mutate(pred = pawar_2018(temp, r_tref, e, eh, topt, tref = 15))

# calculate bootstrapped confidence intervals
boot_conf_predsi <- group_by(boot_predsi, temp) %>%
  summarise(conf_lower = quantile(pred, 0.025),
            conf_upper = quantile(pred, 0.975)) %>%
  ungroup()
...
ggplot() +
  geom_line(aes(temp, .fitted), cz_preds, col = 'blue') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), boot_conf_predsi, fill = 'blue', alpha = 0.3) +
  geom_point(aes(temp, rate), tm.f, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (C)')

# calculate params with CIs
extra_paramsi <- calc_params(fit_nlsLMi) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_paramsi <- Boot(fit_nlsLMi, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(fit_nlsLMi)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
ci_extra_paramsi <- left_join(ci_extra_paramsi, extra_paramsi)

#- Topt 
topti <- as_tibble(ci_extra_paramsi[2,])
topti$species <- as.character("Tetraneura nigri abdominalis")

ggplot(topti, aes(estimate, species)) +
  geom_point(size = 4) + 
  geom_linerange(aes(xmin = conf_lower, xmax = conf_upper)) +
  theme_bw(base_size = 12) +
  scale_x_continuous('') +
...
T.Tx <- rbind(toptg,topth,topti)

T.Tx$param <- as.character(c("1/development time","juvenile mortality rate","fecundity rate"))

T.Tx$euler_param <- as.character(c("1/alpha","zj","bpk"))

T.Tx$stage <- as.factor(c("juvenile","juvenile","adult"))

jqq <- rbind(moz11,jq,T.Tx)

mnuniquefinalmoz <- unique(jqq)
...
#write.csv(mnuniquefinalmoz, 'eulerparammnfin.csv')
