...
A_g <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         ...
                         lower = get_lower_lims(a.g$temp, a.g$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(a.g$temp, a.g$rate, model_name = 'pawar_2018'),
...
ttt <- rbind(topt1,topt2,topt3,topt4,topt5,topt6,topt7)
...
B_t <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                          ...
                          lower = get_lower_lims(b.t$temp, b.t$rate, model_name = 'pawar_2018'),
                          upper = get_upper_lims(b.t$temp, b.t$rate, model_name = 'pawar_2018'),
...
topt9 <- as_tibble(rbind(ci_extra_params9[1,],ci_extra_params9[2,]))
topt9$species <- as.character("Corythucha ciliata")


#€€€€€ Helicoverpa armigera

h.a <- dv %>% filter(curve_ID == 10)

H_a <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = h.a,
                         start = coef(dv_fits$fit[[10]]),
                         lower = get_lower_lims(h.a$temp, h.a$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(h.a$temp, h.a$rate, model_name = 'pawar_2018'),
...
topt10 <- as_tibble(rbind(ci_extra_params10[1,],ci_extra_params10[2,]))
...
topt11 <- as_tibble(rbind(ci_extra_params11[1,],ci_extra_params11[2,]))
...
s.p <- dv %>% filter(curve_ID == 16)

S_p <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = s.p,
                         start = coef(dv_fits$fit[[16]]),
                         lower = get_lower_lims(s.p$temp, s.p$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(s.p$temp, s.p$rate, model_name = 'pawar_2018'),
...
topt16 <- as_tibble(rbind(ci_extra_params15[1,],ci_extra_params15[2,]))
topt16$species <- as.character("Stethorus punctillum")

#€€€€€ Telenomus isis

t.i <- dv %>% filter(curve_ID == 17)

T_i <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = t.i,
                         start = coef(dv_fits$fit[[17]]),
                         lower = get_lower_lims(t.i$temp, t.i$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(t.i$temp, t.i$rate, model_name = 'pawar_2018'),
...
c.a <- dv %>% filter(curve_ID == 20)

C_a <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = c.a,
                         lower = get_lower_lims(c.a$temp, c.a$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(c.a$temp, c.a$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[20]]),
                         weights = rep(1, times = nrow(c.a)))


extra_params20 <- calc_params(C_a) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params20 <- Boot(C_a, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(C_a)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt20 <- as_tibble(rbind(ci_extra_params20[1,],ci_extra_params20[2,]))
topt20$species <- as.character("Culex annulirostris")

#€€€€€ Cydia pomonella

c.p <- dv %>% filter(curve_ID == 21)

C_p <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = c.p,
                         lower = get_lower_lims(c.p$temp, c.p$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(c.p$temp, c.p$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[21]]),
                         weights = rep(1, times = nrow(c.p)))


extra_params21 <- calc_params(C_p) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params21 <- Boot(C_p, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(C_p)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt21 <- as_tibble(rbind(ci_extra_params21[1,],ci_extra_params21[2,]))
topt21$species <- as.character("Cydia pomonella")


#€€€€€ Lepinotus reticulatus

l.r <- dv %>% filter(curve_ID == 22)

L_r <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = l.r,
                         lower = get_lower_lims(l.r$temp, l.r$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(l.r$temp, l.r$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[22]]),
                         weights = rep(1, times = nrow(l.r)))


extra_params22 <- calc_params(L_r) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params22 <- Boot(L_r, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(L_r)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt22 <- as_tibble(rbind(ci_extra_params22[1,],ci_extra_params22[2,]))
topt22$species <- as.character("Lepinotus reticulatus")


#€€€€€ Trichogramma bruni

t.b <- dv %>% filter(curve_ID == 23)

T_b <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = t.b,
                         lower = get_lower_lims(t.b$temp, t.b$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(t.b$temp, t.b$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[23]]),
                         weights = rep(1, times = nrow(t.b)))


extra_params23 <- calc_params(T_b) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params23 <- Boot(T_b, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(T_b)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt23 <- as_tibble(rbind(ci_extra_params23[1,],ci_extra_params23[2,]))
topt23$species <- as.character("Trichogramma bruni")


#€€€€€ Trichogramma sp. nr. Lutea

t.l <- dv %>% filter(curve_ID == 24)

T_l <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = t.l,
                         lower = get_lower_lims(t.l$temp, t.l$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(t.l$temp, t.l$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[24]]),
                         weights = rep(1, times = nrow(t.l)))


extra_params24 <- calc_params(T_l) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params24 <- Boot(T_l, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(T_l)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt24 <- as_tibble(rbind(ci_extra_params24[1,],ci_extra_params24[2,]))
topt24$species <- as.character("Trichogramma sp. nr. Lutea")

#€€€€€ Macrocentrus iridescens

m.i <- dv %>% filter(curve_ID == 25)

M_i <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = m.i,
                         lower = get_lower_lims(m.i$temp, m.i$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(m.i$temp, m.i$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[25]]),
                         weights = rep(1, times = nrow(m.i)))


extra_params25 <- calc_params(M_i) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params25 <- Boot(M_i, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(M_i)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
topt25 <- as_tibble(rbind(ci_extra_params25[1,],ci_extra_params25[2,]))
topt25$species <- as.character("Macrocentrus iridescens")



#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

dv_tz <- rbind(tttz, topt11,topt12,topt13,topt14,
               topt15,topt16,topt17,topt18,topt19,
               topt20,topt21,topt22,topt23,topt24,topt25)

dv_tz$trait <- c("juvenile development rate")
...
write.csv(dv_tz, "dvrate_topts.csv")
