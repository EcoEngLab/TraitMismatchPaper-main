...
ttt <- rbind(topt1,topt2,topt4,topt5,topt6,topt7)
...
topt16 <- as_tibble(rbind(ci_extra_params16[1,],ci_extra_params16[2,]))
topt16$species <- as.character("Stethorus punctillum")

#€€€€€ Telenomus isis

t.i <- dv %>% filter(curve_ID == 15)
...
dv_tz <- rbind(tttz, topt11,topt12, topt13,topt15,topt16,topt17,topt18,topt19)

dv_tz$trait <- c("juvenile mortality rate")

dv_topt <- dv_tz %>% filter(param =="topt")
dv_rm   <- dv_tz %>% filter(param =="rmax")
dv_rm$temp <- dv_topt$estimate
dv_rm$estimate <- 1/dv_rm$estimate
...
write.csv(dv_tz, "jmortrate_topts.csv")
