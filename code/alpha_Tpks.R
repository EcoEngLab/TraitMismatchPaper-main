...
t.sp <- dv %>% filter(curve_ID == 24)

T_sp <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                         data = t.sp,
                         lower = get_lower_lims(t.sp$temp, t.sp$rate, model_name = 'pawar_2018'),
                         upper = get_upper_lims(t.sp$temp, t.sp$rate, model_name = 'pawar_2018'),
                         start = coef(dv_fits$fit[[19]]),
                         weights = rep(1, times = nrow(t.sp)))


extra_params24 <- calc_params(T_sp) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params24 <- Boot(T_sp, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(T_sp)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
m.ir <- dv %>% filter(curve_ID == 25)

M_ir <- minpack.lm::nlsLM(rate~pawar_2018(temp = temp, r_tref,e,eh,topt, tref = 15),
                          data = m.ir,
                          lower = get_lower_lims(m.ir$temp, m.ir$rate, model_name = 'pawar_2018'),
                          upper = get_upper_lims(m.ir$temp, m.ir$rate, model_name = 'pawar_2018'),
                          start = coef(dv_fits$fit[[20]]),
                          weights = rep(1, times = nrow(m.ir)))


extra_params25 <- calc_params(M_ir) %>%
  pivot_longer(everything(), names_to =  'param', values_to = 'estimate')

ci_extra_params25 <- Boot(M_ir, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(M_ir)), R = 200, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
  rename(conf_lower = 1, conf_upper = 2) %>%
  rownames_to_column(., var = 'param') %>%
...
dv_tz <- rbind(topt1,topt2,topt3,topt4,topt5,
               topt6,topt7,topt8,topt12,topt13,
               topt14,topt15,topt16,topt17,topt18,
               topt19,topt20,topt23,topt24,topt25)
...
write.csv(dv_tz, "alpha_Tpks.csv")
